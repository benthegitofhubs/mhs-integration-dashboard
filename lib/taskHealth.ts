export type TaskHealth = "On Track" | "At Risk" | "Blocked" | "Off Track";
...
export function calcTaskHealth(
  ...
  if (task.status === "Blocked") {
    return { status: "Blocked", paceGap: null, reason: "Manually flagged as Blocked" };
  }

  if (task.status === "At Risk") {
    return { status: "At Risk", paceGap: null, reason: "Manually flagged as At Risk" };
  }
...
export const HEALTH_META: Record<TaskHealth, { bg: string; color: string; dot: string }> = {
  ...
  "Blocked":   { bg: "#ffedd5", color: "#c2410c", dot: "#ea580c" },
